...
public class SkuServiceImpl implements SkuService {
    ...
    public void decrCount(String username) {
        //获取购物车中的商品列表
        List<OrderItem> orderIterms = redisTemplate.boundHashOps("cart_" + username).values();
        if (orderIterms != null && orderIterms.size() > 0) {
            for (OrderItem orderIterm : orderIterms) {
                int count = skuMapper.decrCount(orderIterm.getSkuId(), orderIterm.getNum());
                // sql语句执行失败的，超卖了
                if (count <= 0) {
